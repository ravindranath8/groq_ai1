...
GROQ_API_KEY = os.getenv('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')
...
def save_chat_history():
    """Save chat history to a file (CSV or JSON)."""
    if st.button("💾 Save Chat History", use_container_width=True):
        file_type = st.radio("Choose file type", ("CSV", "JSON"))

        if file_type == "CSV":
            save_chat_history_as_csv()
        elif file_type == "JSON":
            save_chat_history_as_json()

def save_chat_history_as_csv():
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        for message in st.session_state.chat_history:
            writer.writerow([message['human'], message['AI']])
    st.success(f"Chat history saved as {filename}")

def save_chat_history_as_json():
